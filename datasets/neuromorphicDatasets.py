...
def cal_fixed_frames_number_segment_index(events_t: np.ndarray, split_by: str, frames_num: int,
    ...
    if split_by == 'number':
        di = N // ((1 - overlap) * (frames_num - 1) + 1)
        for i in range(frames_num):
            if i == 0:
                j_l[i] = 0
            else:
                j_l[i] = j_r[i - 1] - di * overlap
            j_r[i] = j_l[i] + di
        j_r[-1] = N
...
def integrate_events_by_fixed_frames_number(events: Dict, split_by: str, frames_num: int, H: int, W: int,
    ...
    j_l, j_r = cal_fixed_frames_number_segment_index(t, split_by, frames_num, overlap=overlap)
...
def integrate_events_file_to_frames_file_by_fixed_frames_number(loader: Callable, events_np_file: str, output_dir: str,
                                                                ...
                                                                overlap: float, print_save: bool = False) -> None:
    '''
...
def pad_sequence_collate(batch: list):
    ...
    return torch.nn.utils.rnn.pad_sequence(x_list, batch_first=True), torch.as_tensor(y_list), torch.as_tensor(
        x_len_list)
    ...
    def __init__(
        ...
        if data_type == 'event':
            ...
            if frames_number is not None:
                ...
                frames_np_root = os.path.join(root,
                                              f'frames_number_{frames_number}_overlap_{overlap}_split_by_{split_by}')
                            ...
                            if e_files.__len__() > 0:
                                ...
                                for e_file in e_files:
                                    ...
                                    tpe.submit(integrate_events_file_to_frames_file_by_fixed_frames_number,
                                               self.load_events_np, events_np_file, output_dir, split_by, frames_number,
                                               H, W, overlap, True)
                            ...
                            if e_files.__len__() > 0:
                                ...
                                for e_file in e_files:
                                    ...
                                    tpe.submit(integrate_events_file_to_frames_file_by_fixed_duration,
                                               self.load_events_np, events_np_file, output_dir, duration, H, W, True)
                        ...
                        for e_root, e_dirs, e_files in os.walk(events_np_root):
                            if e_files.__len__() > 0:
                                ...
                                for e_file in e_files:
                                    ...
                                    print(
                                        f'Start to integrate [{events_np_file}] to frames and save to [{output_dir}].')
                                    tpe.submit(save_frames_to_npz_and_print,
                                               os.path.join(output_dir, os.path.basename(events_np_file)),
                                               custom_integrate_function(np.load(events_np_file), H, W))
